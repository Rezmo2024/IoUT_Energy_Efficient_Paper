...
class Main():
    ...
    def Create_Topology(self,cor_file,sinkid):
        self.Topology=self.LoadTo_Networkx(TRANSMISSION_RANGE,cor_file)
            ...
            for node2 in G.nodes():
                if node1 != node2:
                    ...
                    if distance <= Range and distance>0:
                        ...
                        link_reliability=self.calculate_Link_Reliability(self.PACKET_SIZE,self.calculate_gamma(TRANSMISSION_POWER,TRANSMISSION_RANGE,distance,FREQUENCY))
    ...
    def create_graph_dictionary(self,G,field):
        ...
        for edge in G.edges(data=True):
            ...
            delay_dic[str(node1)][str(node2)] = weight
            delay_dic[str(node2)][str(node1)] = weight
        return delay_dic        
    ...
    def Calculate_Paths(self):
            ...
            for item in self.srcdest_pairs:
                ...
                d=item[1]
                path=None
                if self.method=="NONSDNHopCountShortestPath":
                    ...
                    path = nx.shortest_path(self.Topology, source=s, target=d)#hop count
                elif self.method=="Energybased":
                        self.Eliminate_DeadNodes(topo1)
                        #path=ILP_MO_Weighted.CalculatePath(s,d,topo1,topo2,alpha=0.5)#because this takes more time than dijkstra I disable it for future
                        En_dic = {str(item[0]): item[1] for item in self.Nodes_Energy}
                        Power_Transmit_a_Packet=(self.PACKET_SIZE/BANDWIDTH)*TX_ENERGY
                        Power_Recive_a_Packet=(self.PACKET_SIZE/BANDWIDTH)*RX_ENERGY
                        #print(En_dic,Power_Recive_a_Packet,Power_Transmit_a_Packet)
                        #path=ILP_Solver.calculate_path(str(s),str(d),topo1,En_dic,self.PACKET_RATE,self.PACKET_SIZE,Power_Transmit_a_Packet,Power_Recive_a_Packet)
                        path=ILP_Heuristic_Solver.heuristic_minimize_delta(topo1,En_dic,str(s),str(d),self.PACKET_RATE,self.PACKET_SIZE,Power_Transmit_a_Packet,Power_Recive_a_Packet)
                        if path!=None:
